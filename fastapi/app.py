...
class FastAPI:
    ...
    def delete(self, path: str, *, response_model: Any = None, status_code: int = 200):
        def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
            self._routes.append(
                Route("DELETE", path, handler=func, status_code=status_code, expects_body=False)
            )
            return func
    ...
    def handle_request(self, method: str, path: str, body: Optional[Dict[str, Any]] = None) -> Any:
        ...
        query_params = {key: values[-1] for key, values in parse_qs(query_string, keep_blank_values=True).items()}
        for route in self._routes:
            if route.method != method:
                continue
            params = route.match(path_only)
            if params is None:
                continue
            ...
            try:
                ...
                return route.status_code, route.handler(*ordered_params, **query_params)
